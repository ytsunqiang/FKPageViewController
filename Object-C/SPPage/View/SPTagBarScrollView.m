...
- (void)__setupTagBarView
{
    ...
    for (int i=0;i<[self.tabDataSource numberOfTab];i++) {
        ...
        titleView.normalFont = [self.tabDataSource respondsToSelector:@selector(titleFontForIndex:)]? [self.tabDataSource titleFontForIndex:i]:[UIFont systemFontOfSize:15.0];
        titleView.title.font = titleView.normalFont;
        titleView.selectedFont = [self.tabDataSource respondsToSelector:@selector(titleFontForSelectedIndex:)]?[self.tabDataSource titleFontForSelectedIndex:i]:titleView.normalFont;
...
- (void)__setupMaskView
{
    ...
    if ([self needMarkView]) {
        ...
        CGFloat h = [self.tabDataSource respondsToSelector:@selector(markViewHeight)]?[self.tabDataSource markViewHeight]:2;
        CGFloat y = self.frame.size.height - h - ([self.tabDataSource respondsToSelector:@selector(markViewBottom)]?[self.tabDataSource markViewBottom]:-13);
        self.markView.frame = CGRectMake(0, y, [self.tabDataSource markViewWidthForIndex:self.index], h);

        self.markView.layer.cornerRadius = self.markView.frame.size.height * 0.5;
    ...
    if (![self needMarkView]) {
        ...
        if (animated) {
            ...
            [UIView animateWithDuration:0.3 animations:^{
                ...
                CGRect frame = bScrollView.markView.frame;
                frame.size.width = [bScrollView.tabDataSource markViewWidthForIndex:index];
                bScrollView.markView.frame = frame;
...
- (void)markViewScrollToContentRatio:(CGFloat)contentRatio
{
    ...
    CGFloat offset = (contentRatio-fromIndex)*(nextTagView.center.x-curTagView.center.x);
    CGFloat moveCenterX = curTagView.center.x+offset;
    ...
    CGFloat change = (toWidth - fromWidth) * offset / (nextTagView.center.x - curTagView.center.x);
    CGRect frame = self.markView.frame;
    frame.size.width = fromWidth + change;
...
- (void)markViewScrollToIndex:(NSInteger)index
{
    ...
    CGRect frame = self.markView.frame;
    frame.size.width = [self.tabDataSource markViewWidthForIndex:index];
    self.markView.frame = frame;
