...
- (void)__setupTabBar
{
    ...
    if (!tabView) {
        CGRect frame = [self preferTabFrame];
        tabView = [[SPTagBarScrollView alloc] initWithFrame:frame dataSource:self delegate:self];
    }
...
- (void)__setupLineView {
    
        if ([self needLineView]) {
            self.lineView = [[UIView alloc] initWithFrame:CGRectMake(0,[self preferTabY] + [self preferTabHAtIndex:0]  - 1, SCREEN_WIDTH ,1)];
            self.lineView.backgroundColor = [self bottomLineViewColor];
        }
